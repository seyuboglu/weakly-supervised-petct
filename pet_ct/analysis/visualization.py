...
def overlay_imgs(ct, pet, resolution=None, saliency=None, alpha=0.5):
    """
    """
...
def show_attention(attention_probs, token_idx=None, threshold=0.0007):
    ...
    for head_idx in range(attention_probs.shape[1]):
        ...
        all_data = [(z, y, x, float(attention_head[z, y, x]))
        ...
        z, y, x, att = zip(*[(z, y, x, att) for z, y, x, att in all_data if att > threshold])
        trace2 = go.Scatter3d(
            x=x,
            y=z,
            z=y,
...
def show_saliency_maps(saliency_maps, name, video=True, top_slices=0, resolution=None):
    ...
    for j in tqdm(range(images.shape[1])):
        ...
        image_saliency = overlay_imgs(image_tensor[:, :, 0], image_tensor[:, :, 1], saliency=saliency)
...
def plot_saliency_curve(saliency_maps, dim=0):
    ...
    for name, saliency_map in saliency_maps.items():
        ...
        if dim == 0:
            average_saliency = np.array([saliency_tensor[0, j, :, :].mean()
        ...
        data.append(go.Scatter(x=np.arange(len(average_saliency)),
                               y=average_saliency,
                               name=f"{name}"))
    layout = dict(title = "Average saliency over frames",
...
def show_3d_saliency(saliency_maps, tasks=None):
    ...
    for idx, (name, saliency_map) in enumerate(saliency_maps.items()):
        ...
        scan_grad = torch.nn.functional.max_pool3d(scan_grad,
                                                   kernel_size=(7, 21, 21)).squeeze()
        ...
        all_data = [(z, y, x, float(saliency_tensor[z, y, x]))
                    for z in range(len_z)
                    for y in range(len_y)
                    for x in range(len_x)]
...
def plot_training_curve(epoch_data, metric, tasks=["primary"], splits=None):
    ...
    for idx, task in enumerate(tasks):
        if splits is not None:
            for split in splits:
                line = {
                    "color": "rgba" + color_scale[idx % 8][3:-1] + (",1.0)" if split == "valid" else ",0.4)")
                }
                col = f"{split}_{task}_{metric}"
                data.append(go.Scatter(x=epoch_data.df.index,
                                       y=epoch_data.df[col],
                                       name=f"{task}_{split}",
                                       line=line))
        else:
    ...
    layout = dict(title=f"{task}-{metric}",
...
def plot_learning_curves(experiment_dirs, metric,
                         tasks=["primary"], splits=["train", "valid"]):
    """
    ...
    if type(experiment_dirs) is list:
        experiments = {os.path.basename(exp_dir): exp_dir
    ...
    for idx, (name, experiment_dir) in enumerate(experiments.items()):
        if os.path.isfile(os.path.join(experiment_dir, "epochs.csv")):
            ...
            if tasks is None and splits is None:
                ...
                data.append(go.Scatter(x=train_history.df.index,
                ...
                for task in tasks:
                    for split in splits:
                        if f"{split}_{task}_{metric}" in train_history.df:
                            line = {
                                # create rgba value
                                "color": ("rgba" + color_scale[idx % 9][3:-1] +
    ...
    layout = dict(title=f"{metric}",
                  xaxis=dict(title='epoch'),
                  yaxis=dict(title=f"{metric}"))
    ply.iplot({"data":data, "layout":layout})


def plot_roc_curves(experiment_dirs, splits=["train", "valid"],
                    tasks=["primary"], epoch="best"):
    """
    Plots the learning curve for a set of experiments specified by experiment_dirs
    Args:
        experiments     (list or dict)  dict from name to experiment dir OR list
    ...
    if type(experiment_dirs) is list:
        experiment_dirs = {os.path.basename(exp_dir): exp_dir
                           for idx, exp_dir in enumerate(experiment_dirs)}
    ...
    for exp_idx, (name, experiment_dir) in enumerate(experiment_dirs.items()):
        for task_idx, task in enumerate(tasks):
            for split in splits:
                ...
                preds_df = pd.read_csv(os.path.join(experiment_dir, f"{epoch}/{split}_preds.csv"),
                                                    index_col=[0], header=[0,1])

                correct_probs = np.array(list(map(lambda x: float(x.strip('[]')),
                                        preds_df[task, "correct_prob"].values[1:])))
                targets = np.array(list(map(lambda x: int(x.strip('[]')),
...
def plot_roc_curve(experiment_dir, split="valid", name="best", task="primary"):
    """
    """
    preds_df = pd.read_csv(os.path.join(experiment_dir, f"{name}/{split}_preds.csv"),
                                        index_col=[0], header=[0,1])
...
def plot_metric_scatter(experiment_dir, stats_path, stat,
                        split="valid", metric="roc_auc", name="last", tasks={}):
...
def compare_experiments(experiments=[], search_subdirs=True, metric="roc_auc",
                        epoch="last", splits=["valid"], tasks=None, direct_compare=False,
                        package=None, save_path=None):
    """
    """
    ...
    def add_metrics(experiment_dir, name):
        if type(epoch) == dict:
            ...
            exp_epoch = epoch
        for split in splits:
            metrics_path = os.path.join(experiment_dir, f"{exp_epoch}/{split}_metrics.json")
            ...
            for task, metrics in metrics_dict.items():
                ...
                entry = {
                    ...
                    "task": task, #params_dict["process_args"]["dataset_args"]["class_boundaries"][0],
    ...
    if package == "plotly":
        ...
        fig = px.bar(mean_data_df, x="task", y=f"{metric}", color=f'name', barmode='group',
                     height=400)
        ...
        mean_data_df = data_df.groupby(['task']).mean().reset_index()
        order = mean_data_df.sort_values(f"{metric}")["task"]
        f = sns.stripplot(data=data_df, x="task", y=f"{metric}",
                      hue=f'name', order=order,
                      linewidth=4, size=12)
        sns.despine()
        ...
        print(data_df)
        f = sns.stripplot(data=data_df, x="task", y=f"{metric}", 
                          hue=f'name', dodge=True, order=order,
                          linewidth=0, size=12, jitter=0, alpha=0.4, zorder=1)
        f = sns.barplot(data=data_df, x="task", y=f"{metric}", 
                          hue=f'name', order=order, alpha=0, color="grey", capsize=0.05, zorder=10)
        f = sns.barplot(data=data_df, x="task", y=f"{metric}", ci=None,
                          hue=f'name', order=order, alpha=1, color="grey", capsize=0.05, zorder=0)
...
def compare_experiment_groups(experiment_groups, search_subdirs=True, metric="roc_auc",
                              epoch="last", splits=["valid"], tasks=None, direct_compare=False,
                              package=None, save_path=None):
    """
    """
    experiments = unpack_experiment_groups(experiment_groups)
    return compare_experiments(experiments, search_subdirs, metric,
                               epoch, splits, tasks, direct_compare, package, save_path)


def show_saliency_volume(saliency_maps, tasks=[], show_ct=True, show_pet=False,
                         colormap=None):
    ...
    def add_controls(volume, name, color=None):
        """
        """
        level = FloatLogSlider(base=10, min=-0.5, max=0, step=0.0002, description=f"{name} level:")
        opacity = FloatLogSlider(base=10, min=-2, max=1.0, step=0.01, description=f"{name} opacity:")
    ...
    for idx, task in enumerate(tasks):
        ...
        scan_grad = scan_grad.squeeze() #torch.nn.functional.max_pool3d(scan_grad, kernel_size=(7, 21, 21)).squeeze()
        ...
        saliency_vol = ipv.volshow(saliency_tensor, downscale=100, level=(0.5, 0.5, 0.5), 
                                   opacity=opacity, controls=True)
    ...
    if show_ct:
        ct = scan[:, :, :, :, 0].squeeze()
        data["ct"] = ct
        ct_vol = ipv.volshow(ct, downscale=100, level=(1.0, 1.0, 1.0), opacity=(0.2, 0.2, 0.2))
        add_controls(ct_vol, name="ct")
    if show_pet:
        pet = scan[:, :, :, :, 1].squeeze()
        data["pet"] = pet
        opacity = (np.array((228,26,28)) / 256) * 0.2
        pet_vol = ipv.volshow(pet, downscale=100, level=(0.7, 0.7, 0.7), opacity=opacity)
...
def show_attention_volume(scan, attention_maps, tasks=[], show_ct=True, show_pet=False,
                          downscale=1, exam_id="", movie_dir=None, colormap=None,
                          flip_axes=None):
    ...
    def add_controls(volume, name, color=None):
        """
        """
        level = FloatLogSlider(base=10, min=-0.5, max=0, step=0.01, description=f"{name} level:")
        opacity = FloatLogSlider(base=10, min=-2, max=0.6, step=0.01, description=f"{name} opacity:")
        jslink((volume.tf, 'level1'), (level, "value"))
        jslink((volume.tf, 'level2'), (level, "value"))
        jslink((volume.tf, 'level3'), (level, "value"))
        jslink((volume, 'opacity_scale'), (opacity, "value"))
    ...
    for idx, task in enumerate(tasks):
        ...
        saliency_vol = ipv.volshow(attention_map, level=(1.0, 1.0, 1.0),
                                   opacity=opacity, controls=True,
                                   extent=[[0, attention_map.shape[0]],
    ...
    if show_ct:
        ...
        ct_vol = ipv.volshow(ct, downscale=100, level=(0.7, 0.7, 0.7),
                             opacity=(0.2, 0.2, 0.2),
                             extent=[[0, ct.shape[0]],
                                     [0, ct.shape[1]],
                                     [0,ct.shape[2]]])
