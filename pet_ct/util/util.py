...
def write_params(process_dir, params):
    ...
    with open(os.path.join(process_dir, "params.json"), "w") as f:
        ...
        json.dump(params, f, indent=4, default=default)
...
def process(fn):
    ...
    def with_logging(*args, **kwargs):
        ...
        if args_dict["process_dir"] is not None:
            ...
            set_logger(
                os.path.join(args_dict["process_dir"], "process.log"),
                level=logging.INFO,
                console=True,
            )
            params_dict = {
...
class Process:
    ...
    def run(self, overwrite=False, notify=False, commit=False, **kwargs):
        ...
        if os.path.isfile(os.path.join(self.dir, "stats.csv")) and not overwrite:
            ...
            return False
        if notify:
            self.notify_user(event="start")
            try:
                self._run(overwrite, **kwargs)
            except:
                tb = traceback.format_exc()
                self.notify_user(event="error", msg=tb)
                return False
            else:
                self.notify_user()
                return True
    ...
    def notify_user(self, event="completion", msg=None):
        """
...
def soft_to_hard(Y_s, break_ties="random"):
    ...
    for i in range(n):
        max_idxs = torch.where(
            diffs[i, :] < TOL, Y_s[i], torch.tensor(0.0, dtype=Y_s.dtype)
        )
...
def log_cuda_memory(devices=[0], units="g", ndigits=4):
    ...
    for device in devices:
        ...
        max_allocated = round(
            torch.cuda.max_memory_allocated(device) * conversion, ndigits
        )
        logging.info(
            f"Memory allocated: {allocated} {units}  / {max_allocated} {units} "
        )
...
def log_predictions(targets, predictions, info=None):
    ...
    if "report_generation" in predictions:
        for idx, (target, pred) in enumerate(
            zip(targets["report_generation"], predictions["report_generation"])
        ):
            if info is not None:
...
def create_notebook(
    experiment_dir, notes=None, template_path="notebooks/experiment_template.ipynb"
):
    """
    """
